...
export const zh_CN: TranslationMap = {
  ...
  config: {
    ...
    status: {
      unsavedRaw: "有未保存更改",
      unsavedCountOne: "{count} 项未保存更改",
      unsavedCountMany: "{count} 项未保存更改",
      noChanges: "无更改",
    },
    actions: {
      loading: "加载中...",
    ...
    diff: {
      pendingCountOne: "查看 {count} 项待应用更改",
      pendingCountMany: "查看 {count} 项待应用更改",
    },
    loading: {
      schema: "正在加载 schema...",
    },
    warnings: {
      formUnsafe: "表单视图无法安全编辑部分字段。请改用 Raw 模式，避免配置项丢失。",
    },
    raw: {
      title: "原始 JSON5",
    },
    ...
    node: {
      ...
      arrayCountOne: "{count} 项",
      arrayCountMany: "{count} 项",
      add: "添加",
      noItemsYet: "暂无条目。点击“添加”创建一项。",
      removeItem: "删除该项",
      customEntries: "自定义条目",
      addEntry: "添加条目",
    ...
    sections: {
      ...
      bindings: { label: "绑定", description: "快捷键与静态路由绑定" },
      ...
      talk: { label: "语音", description: "语音与播报设置" },
      plugins: { label: "插件", description: "插件管理与扩展" },
    },
    fields: {
      env: {
        label: "环境变量",
        help: "用于向网关进程提供运行时变量的环境导入与覆盖设置。可在此控制 shell 环境加载和显式变量注入行为。",
        shellEnv: {
          label: "Shell 环境导入",
          help: "控制启动时是否从你的登录 shell 导入环境变量。当你依赖 shell 配置中的密钥或 PATH 自定义时，建议保持启用。",
          enabled: {
            label: "启用 Shell 环境导入",
            help: "启用后，启动初始化阶段会从用户 shell 配置中加载环境变量。开发机器建议开启；在严格受控环境中可关闭并改为显式环境管理。",
          },
          timeoutMs: {
            label: "Shell 环境导入超时（毫秒）",
            help: "解析 shell 环境的最大等待时间（毫秒）。想要更快启动可减小；若 shell 初始化较重可适当增大。",
          },
        },
        vars: {
          label: "环境变量覆盖",
          help: "显式 key/value 环境变量覆盖，会合并到 OpenClaw 运行时进程环境中。用于获得更可控、可复现的环境配置。",
        },
      },
      update: {
        label: "更新",
        help: "用于保持 OpenClaw 运行版本最新的更新通道与启动检查设置。生产环境建议使用稳健通道，实验环境再启用更激进设置。",
        channel: {
          label: "更新通道",
          help: "用于 git + npm 安装的更新通道（stable、beta 或 dev）。",
        },
        checkOnStart: {
          label: "启动时检查更新",
        ...
        auto: {
          ...
          enabled: {
            label: "启用自动更新",
            help: "是否启用后台自动更新包安装（默认：false）。",
          },
        },
      },
      agents: {
        label: "代理",
        help: "代理运行时默认项与按代理覆盖项，包括模型、工作区、工具行为及会话策略。",
        defaults: {
          label: "代理默认设置",
          help: "共享默认设置；除非在 agents.list 中单独覆盖，否则各代理会继承这里的配置。",
        },
        list: {
          label: "代理列表",
          help: "用于定义每个代理的差异化配置，例如工作区、模型、技能和路由策略。",
        },
      },
      auth: {
        label: "认证",
        help: "OpenClaw 模型提供商使用的认证配置档与故障转移/冷却策略。",
        profiles: {
          label: "认证配置档",
          help: "命名的认证配置（provider + mode + 可选 email）。",
        },
        profileOrder: {
          label: "认证配置档顺序",
          help: "按提供商设置认证配置档顺序（用于自动故障转移）。",
        },
      },
    },
  },
