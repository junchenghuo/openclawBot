...
function pathI18nKey(path: Array<string | number>): string {
  return path
...
function resolveFieldMeta(
  ...
  const pathKeyValue = pathI18nKey(path);
  const fallbackLabel = hint?.label ?? schema.title ?? humanize(String(path.at(-1)));
  const fallbackHelp = hint?.help ?? schema.description;
  const label = localizeConfigFieldLabel(pathKeyValue, fallbackLabel);
  const help = localizeConfigFieldHelp(pathKeyValue, label, fallbackHelp);
...
export function renderNode(params: {
  ...
  if (unsupported.has(key)) {
    return html`<div class="cfg-field cfg-field--error">
      ...
      <div class="cfg-field__error"
        >${tr("config.node.unsupportedSchemaNode", "Unsupported schema node. Use Raw mode.")}</div
      >
    </div>`;
  }
  if (
  ...
  return html`
    <div class="cfg-field cfg-field--error">
      <div class="cfg-field__label">${label}</div>
      <div class="cfg-field__error"
        >${tr("config.node.unsupportedType", "Unsupported type: {type}. Use Raw mode.", {
          type: String(type),
        })}</div
      >
    </div>
      ...
      <div class="cfg-input-wrap">
        ...
        ${
          ...
          <button
            ...
            title=${tr("config.node.resetToDefault", "Reset to default")}
            ?disabled=${disabled}
...
function renderSelect(params: {
  ...
  return html`
    <div class="cfg-field">
      ...
      >
        <option value=${unset}>${tr("config.node.select", "Select...")}</option>
        ${options.map(
          (opt, idx) => html`
  ...
  if (!itemsSchema) {
    return html`
      <div class="cfg-field cfg-field--error">
        <div class="cfg-field__label">${label}</div>
        <div class="cfg-field__error"
          >${tr("config.node.unsupportedArraySchema", "Unsupported array schema. Use Raw mode.")}</div
        >
      </div>
    `;
  }
    ...
    <div class="cfg-array">
      <div class="cfg-array__header">
        ...
        <span class="cfg-array__count"
          >${arr.length === 1
            ? tr("config.node.arrayCountOne", "{count} item", { count: String(arr.length) })
            : tr("config.node.arrayCountMany", "{count} items", {
        ...
        arr.length === 0
          ? html`
              <div class="cfg-array__empty"
                >${tr("config.node.noItemsYet", 'No items yet. Click "Add" to create one.')}</div
              >
            `
          : html`
            ...
            <div class="cfg-array__item">
              <div class="cfg-array__item-header">
                ...
                <button
                  ...
                  title=${tr("config.node.removeItem", "Remove item")}
                  ?disabled=${disabled}
                  @click=${() => {
                    const next = [...arr];
                    next.splice(idx, 1);
  ...
  return html`
    <div class="cfg-map">
      <div class="cfg-map__header">
        <span class="cfg-map__label">${tr("config.node.customEntries", "Custom entries")}</span>
        <button
          type="button"
          class="cfg-map__add"
          ?disabled=${disabled}
        ...
        >
          ...
          ${tr("config.node.addEntry", "Add Entry")}
        </button>
      </div>
      ...
      ${
        visibleEntries.length === 0
          ? html`
              <div class="cfg-map__empty">${tr("config.node.noCustomEntries", "No custom entries.")}</div>
            `
          : html`
                ...
                <div class="cfg-map__item-header">
                  <div class="cfg-map__item-key">
                    <input
                      ...
                      placeholder=${tr("config.node.keyPlaceholder", "Key")}
                      .value=${key}
                      ?disabled=${disabled}
                      @change=${(e: Event) => {
                  ...
                  <button
                    ...
                    title=${tr("config.node.removeEntry", "Remove entry")}
                    ?disabled=${disabled}
                    @click=${() => {
                      const next = { ...value };
                      delete next[key];
                    ...
                    anySchema
                      ? html`
                        <textarea
                          ...
                          placeholder=${tr("config.node.jsonValuePlaceholder", "JSON value")}
                          rows="2"
                          .value=${fallback}
                          ?disabled=${disabled}
                          @change=${(e: Event) => {
