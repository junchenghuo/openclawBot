...
function pluralLabel(params: {
  ...
  return params.count === 1
    ? tr(params.oneKey, params.oneFallback, { count: String(params.count) })
    : tr(params.manyKey, params.manyFallback, { count: String(params.count) });
}
...
function resolveSectionMeta(
  ...
  if (meta) {
    const translatedLabel = localizeConfigSectionLabel(key, meta.label);
    return {
      label: translatedLabel,
      description: localizeConfigSectionDescription(key, translatedLabel, meta.description),
    };
  }
  const fallbackLabel = schema?.title ?? humanize(key);
  const fallbackDescription = schema?.description ?? "";
  const translatedLabel = localizeConfigSectionLabel(key, fallbackLabel);
  return {
    label: translatedLabel,
    description: localizeConfigSectionDescription(key, translatedLabel, fallbackDescription),
  };
}
...
function resolveSubsections(params: {
  ...
  const entries = Object.entries(schema.properties).map(([subKey, node]) => {
    ...
    const fallbackLabel = hint?.label ?? node.title ?? humanize(subKey);
    const fallbackDescription = hint?.help ?? node.description ?? "";
    const label = localizeConfigFieldLabel(fieldPath, fallbackLabel);
    const description = localizeConfigFieldHelp(fieldPath, label, fallbackDescription) ?? "";
...
export function renderConfig(props: ConfigProps) {
  ...
  const validityLabel = tr(`config.validity.${validity}`, validity);
    ...
    <div class="config-layout">
      ...
      <aside class="config-sidebar">
        <div class="config-sidebar__header">
          <div class="config-sidebar__title">${tr("config.sidebar.title", "Settings")}</div>
          <span
            class="pill pill--sm ${
        ...
        <div class="config-search">
          <div class="config-search__input-row">
            ...
            <input
              ...
              placeholder=${tr("config.search.placeholder", "Search settings...")}
          ...
          <div class="config-search__hint">
            <span class="config-search__hint-label" id="config-tag-filter-label"
              >${tr("config.search.tagFilters", "Tag filters:")}</span
            >
                  ...
                  selectedTags.size === 0
                    ? html`
                        <span class="config-search__tag-placeholder"
                          >${tr("config.search.addTags", "Add tags")}</span
                        >
                      `
                    : html`
                            ...
                            .map(
                              (tag) =>
                                html`<span class="config-search__tag-chip"
                                  >tag:${localizeConfigTag(tag)}</span
                                >`,
                            )}
                          ${
                            selectedTags.size > 2
        ...
        <nav class="config-nav">
          ...
          >
            ...
            <span class="config-nav__label"
              >${tr("config.sidebar.allSettings", "All Settings")}</span
            >
          </button>
          ${allSections.map(
            (section) => html`
              ...
              >
                ...
                >
                <span class="config-nav__label"
                  >${resolveSectionMeta(section.key, schemaProps[section.key]).label}</span
                >
              </button>
            `,
          )}
        ...
        <div class="config-sidebar__footer">
          <div class="config-mode-toggle">
            ...
            >
              ${tr("config.mode.form", "Form")}
            </button>
            <button
              class="config-mode-toggle__btn ${props.formMode === "raw" ? "active" : ""}"
              @click=${() => props.onFormModeChange("raw")}
            >
              ${tr("config.mode.raw", "Raw")}
            </button>
          </div>
        </div>
      </aside>
      ...
      <main class="config-main">
        ...
        <div class="config-actions">
              ...
              hasChanges
                ? html`
                  <span class="config-changes-badge"
                    >${
                      props.formMode === "raw"
                        ? tr("config.status.unsavedRaw", "Unsaved changes")
                        : pluralLabel({
                            count: diff.length,
                            oneKey: "config.status.unsavedCountOne",
                            oneFallback: "{count} unsaved change",
                            manyKey: "config.status.unsavedCountMany",
                            manyFallback: "{count} unsaved changes",
                          })
                    }</span
                  >
                `
                : html`
                    <span class="config-status muted"
                      >${tr("config.status.noChanges", "No changes")}</span
                    >
                  `
            }
          </div>
          <div class="config-actions__right">
            ...
            >
              ${props.loading
                ? tr("config.actions.loading", "Loading...")
                : tr("config.actions.reload", "Reload")}
            </button>
            <button
            ...
            >
              ${props.saving
                ? tr("config.actions.saving", "Saving...")
                : tr("config.actions.save", "Save")}
            </button>
            <button
              class="btn btn--sm"
              ?disabled=${!canApply}
            ...
            >
              ${props.applying
                ? tr("config.actions.applying", "Applying...")
                : tr("config.actions.apply", "Apply")}
            </button>
            <button
              class="btn btn--sm"
              ?disabled=${!canUpdate}
            ...
            >
              ${props.updating
                ? tr("config.actions.updating", "Updating...")
                : tr("config.actions.update", "Update")}
            </button>
          </div>
        </div>
              ...
              <details class="config-diff">
                <summary class="config-diff__summary">
                  <span
                    >${pluralLabel({
                      count: diff.length,
                      oneKey: "config.diff.pendingCountOne",
                      oneFallback: "View {count} pending change",
                      manyKey: "config.diff.pendingCountMany",
                      manyFallback: "View {count} pending changes",
                    })}</span
                  >
                  <svg
            ...
            props.formMode === "form"
                  ...
                  props.schemaLoading
                    ? html`
                        <div class="config-loading">
                          ...
                          <span>${tr("config.loading.schema", "Loading schema...")}</span>
                        </div>
                      `
                    : renderConfigForm({
              ...
              : html`
                <label class="field config-raw-field">
                  <span>${tr("config.raw.title", "Raw JSON5")}</span>
                  <textarea
                    .value=${props.raw}
                    @input=${(e: Event) =>
