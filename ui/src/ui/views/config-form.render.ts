...
function resolveSectionMeta(key: string, fallback: { label: string; description: string }) {
  const translatedLabel = localizeConfigSectionLabel(key, fallback.label);
  return {
    label: translatedLabel,
    description: localizeConfigSectionDescription(key, translatedLabel, fallback.description),
  };
}
...
function matchesSearch(params: {
  ...
  const fallbackMeta = SECTION_META[params.key];
  const meta = fallbackMeta ? resolveSectionMeta(params.key, fallbackMeta) : undefined;
...
export function renderConfigForm(props: ConfigFormProps) {
  if (!props.schema) {
    return html`
      <div class="muted">${tr("config.form.schemaUnavailable", "Schema unavailable.")}</div>
    `;
  }
  ...
  if (schemaType(schema) !== "object" || !schema.properties) {
    return html`
      <div class="callout danger"
        >${tr("config.form.unsupportedSchemaUseRaw", "Unsupported schema. Use Raw.")}</div
      >
    `;
  }
    ...
    return html`
      <div class="config-empty">
        ...
        <div class="config-empty__text">
          ${searchQuery
            ? tr(
                "config.form.noSettingsMatch",
                `No settings match "${searchQuery}"`,
              ).replace("{query}", searchQuery)
            : tr("config.form.noSettingsInSection", "No settings in this section")}
        </div>
      </div>
    `;
  }
      ...
      ${
        subsectionContext
          ? (() => {
              ...
              const fieldPath = `${sectionKey}.${subsectionKey}`;
              const fallbackLabel = hint?.label ?? node.title ?? humanize(subsectionKey);
          ...
          : filteredEntries.map(([key, node]) => {
              const fallback = SECTION_META[key] ?? {
                label: key.charAt(0).toUpperCase() + key.slice(1),
                description: node.description ?? "",
              };
              const meta = resolveSectionMeta(key, fallback);
