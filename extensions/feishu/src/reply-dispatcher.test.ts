...
describe("createFeishuReplyDispatcher streaming behavior", () => {
  ...
  it("passes explicit mediaLocalRoots to media attachments", async () => {
    createFeishuReplyDispatcher({
      cfg: {} as never,
      agentId: "agent",
      runtime: {} as never,
      chatId: "oc_chat",
      mediaLocalRoots: ["/tmp/custom-agent-root"],
    });

    const options = createReplyDispatcherWithTypingMock.mock.calls[0]?.[0];
    await options.deliver({ mediaUrl: "./output/test-send.txt" }, { kind: "final" });

    expect(sendMediaFeishuMock).toHaveBeenCalledWith(
      expect.objectContaining({
        mediaUrl: "./output/test-send.txt",
        mediaLocalRoots: ["/tmp/custom-agent-root"],
      }),
    );
  });

  it("derives agent-scoped media roots when not provided", async () => {
    createFeishuReplyDispatcher({
      cfg: {} as never,
      agentId: "pm",
      runtime: {} as never,
    ...
    expect(sendMediaFeishuMock).toHaveBeenCalledWith(
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([
          ...
          "/tmp/openclaw-state/workspace-pm",
        ]),
      }),
    );
  });

  it("falls back to text link when media send fails", async () => {
    sendMediaFeishuMock.mockRejectedValue(new Error("local path denied"));
