...
function resolveReplyMediaLocalRoots(params: {
  cfg: ClawdbotConfig;
  agentId: string;
  stateDir: string;
}): readonly string[] {
  ...
  const roots = new Set<string>([
    resolvePreferredOpenClawTmpDir(),
    path.join(params.stateDir, "media"),
    path.join(params.stateDir, "agents"),
    path.join(params.stateDir, "workspace"),
    path.join(params.stateDir, "sandboxes"),
    path.join(params.stateDir, `workspace-${normalizedAgentId}`),
  ]);
  ...
  // Keep default profile workspaces readable for agents that don't set explicit
  // workspace paths in config.
...
export type CreateFeishuReplyDispatcherParams = {
  ...
  /** Trusted local roots for MEDIA local paths. */
...
export function createFeishuReplyDispatcher(params: CreateFeishuReplyDispatcherParams) {
  ...
  const mediaLocalRoots =
    params.mediaLocalRoots && params.mediaLocalRoots.length > 0
      ...
      : resolveReplyMediaLocalRoots({ cfg, agentId, stateDir });
  const account = resolveFeishuAccount({ cfg, accountId });
  const prefixContext = createReplyPrefixContext({ cfg, agentId });

  const sendMediaWithFallback = async (mediaUrl: string) => {
    try {
      ...
      params.runtime.error?.(
        `feishu[${account.accountId}] media send failed (${mediaUrl}): ${String(error)}`,
      );
