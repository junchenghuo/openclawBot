...
describe("sendMessageMattermost", () => {
  ...
  it("throws when local media upload fails instead of sending text-only message", async () => {
    mockState.loadOutboundMediaFromUrl.mockResolvedValueOnce({
      buffer: Buffer.from("media-bytes"),
      fileName: "photo.png",
      contentType: "image/png",
    ...
    mockState.uploadMattermostFile.mockRejectedValueOnce(new Error("disk read blocked"));

    await expect(
      sendMessageMattermost("channel:town-square", "hello", {
        mediaUrl: "/tmp/disallowed/photo.png",
      }),
    ).rejects.toThrow("Mattermost media upload failed: disk read blocked");

    expect(mockState.createMattermostPost).not.toHaveBeenCalled();
  });

  it("falls back to URL text when remote media upload fails", async () => {
    mockState.loadOutboundMediaFromUrl.mockResolvedValueOnce({
      buffer: Buffer.from("media-bytes"),
      fileName: "photo.png",
      contentType: "image/png",
    ...
    mockState.uploadMattermostFile.mockRejectedValueOnce(new Error("remote timeout"));
