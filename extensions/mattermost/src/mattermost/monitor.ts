...
function isRateLimitAutoReplyLoopMessage(text: string): boolean {
  const normalized = text.trim();
  if (!normalized) {
    return false;
  }
  if (normalized === RATE_LIMIT_AUTO_REPLY_TEXT) {
    return true;
  }
  return /api rate limit reached/i.test(normalized);
}

function looksLikeMattermostBotUsername(username?: string | null): boolean {
  const normalized = username?.trim();
  if (!normalized) {
    return false;
  }
  return /^bot[-_]/i.test(normalized);
}

function normalizeMattermostDisplayName(name?: string | null): string | undefined {
...
export async function monitorMattermostProvider(opts: MonitorMattermostOpts = {}): Promise<void> {
      ...
      try {
        const fetched = await core.channel.media.fetchRemoteMedia({
          ...
          ssrfPolicy: {
            ...
            ...(mattermostHostname ? { allowedHostnames: [mattermostHostname] } : {}),
          },
        });
  ...
  const handlePost = async (
    ...
    const senderUser = await resolveUserInfo(senderId);
    const senderUsername = senderUser?.username?.trim() || undefined;
    const senderNameFromPayload = normalizeMattermostDisplayName(payload.data?.sender_name);
    const senderUsernameLike = senderUsername ?? senderNameFromPayload;
    const senderName = payload.data?.sender_name?.trim() || senderUsername || senderId;
    const rawText = post.message?.trim() || "";

    // Guard against bot-to-bot error ping-pong loops:
    // when one bot mirrors a generic rate-limit text into a DM, the peer bot can
    // treat it as a fresh user message and mirror it back, creating an infinite loop.
    if (
      kind === "direct" &&
      isRateLimitAutoReplyLoopMessage(rawText) &&
      looksLikeMattermostBotUsername(senderUsernameLike)
    ) {
      logVerboseMessage(
        `mattermost: drop dm rate-limit loop message sender=${senderId} username=${senderUsernameLike ?? "unknown"}`,
      );
      return;
    }
