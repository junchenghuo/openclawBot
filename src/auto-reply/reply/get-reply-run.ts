...
function hasOfficeDocumentAttachment(ctx: MsgContext): boolean {
  const mimeEntries = [ctx.MediaType, ...(ctx.MediaTypes ?? [])]
    .map((entry) => entry?.trim().toLowerCase())
    .filter(Boolean) as string[];
  if (
    mimeEntries.some(
      (entry) =>
        entry.includes("application/vnd.openxmlformats-officedocument") ||
        entry === "application/msword" ||
        entry === "application/vnd.ms-powerpoint" ||
        entry === "application/vnd.ms-excel",
    )
  ) {
    return true;
  }

  const pathEntries = [ctx.MediaPath, ...(ctx.MediaPaths ?? [])]
    .map((entry) => entry?.trim().toLowerCase())
    .filter(Boolean) as string[];
  return pathEntries.some((entry) => /\.(docx?|pptx?|xlsx?)$/i.test(entry));
}
...
export async function runPreparedReply(
  ...
  const mediaReplyHint = mediaNote
    ? [
        ...
        ...(hasOfficeDocumentAttachment(ctx)
          ? [
              "For office documents (.doc/.docx/.ppt/.pptx/.xls/.xlsx), do not infer content from raw binary bytes. Use the skill tool first (for example: skill name 'docx' for Word files) to extract text, then summarize only extracted content; if extraction was not performed, explicitly report that the document is unreadable in current context.",
            ]
          : []),
      ].join("\n")
    : undefined;
