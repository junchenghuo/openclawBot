...
describe("runPreparedReply media-only handling", () => {
  ...
  it("adds explicit office-document extraction warning for docx attachments", async () => {
    await runPreparedReply(
      baseParams({
        ctx: {
          ...
          OriginatingTo: "team:channel",
          ChatType: "group",
          MediaPath: "/tmp/input.docx",
    ...
    const call = vi.mocked(runReplyAgent).mock.calls[0]?.[0];
    expect(call).toBeTruthy();
    expect(call?.commandBody).toContain("For office documents (.doc/.docx/.ppt/.pptx/.xls/.xlsx)");
    expect(call?.commandBody).toContain("do not infer content from raw binary bytes");
    expect(call?.commandBody).toContain("skill name 'docx'");
  });
